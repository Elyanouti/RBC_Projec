...
class BlockCommit(BaseBlockCommit):
    ...
    def propose_mutant(node):  # node, i, tx_i
        ...
        if len(block_index) == 1:    # if has only one block, get active transaction id
            old = ref_T.active
            # اختر نسخة جديدة مختلفة عن القديمة
            possible_new = [tx for tx in ref_T.transactions if tx.id != old]
            if possible_new:
                new_active = random.choice(possible_new).id
            else:
                print("No alternative version available for mutation.")
                return
        else:
            ...
            for index in block_index:
                for set in node.blockchain[index].transactions:
                    if set.id == ref_T.id:
                        all_tx.extend([tx for tx in set.transactions if tx.id != ref_T.active])
            if all_tx:
                new_active = random.choice(all_tx).id
            else:
                print("No alternative version available for mutation in multiple blocks.")
                return
